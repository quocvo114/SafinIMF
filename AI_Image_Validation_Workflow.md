...
│  (chỉ kiểm tra ảnh đầu tiên – images[0])│
│                                         │
│  1. Backend decode base64 → buffer      │
│  2. Gửi multipart/form-data → Flask API │
│     (thử lần lượt các URL candidate)   │
│  3. Flask chạy YOLO v11 (conf ≥ 0.25)  │
│  4. Trả về: detections[], total_objects │
└────────┬────────────────────────────────┘
         │
    ┌────┴────────────────────────────┐
    ▼                                 ▼
...
| Bước | Mô tả | Kết quả |
|---|---|---|
| 1 | Người dân chọn ảnh (3–5 ảnh, JPG/PNG, ≤5MB) và điền thông tin báo cáo | Dữ liệu sẵn sàng |
...
| 4 | Backend lấy `images[0]` (ảnh đầu tiên), decode base64 → buffer | Chuẩn bị gửi sang AI service |
| 5 | Backend gọi `verifyImageWithModel()` → thử lần lượt các URL candidate: port 5001 → 5000 → ... | Timeout mỗi URL: 10 giây (env `MODEL_API_TIMEOUT_MS`) |
| 6 | Flask AI service chạy YOLO v11 với `conf=0.25`, phân tích ảnh, trả về `detections[]` + `total_objects` | JSON response |
| 7 | Backend gọi `isAiAccepted()` để đánh giá kết quả AI | Xem tiêu chí bên dưới |
...
| Số lượng ảnh | 3 đến 5 ảnh | Bắt buộc |
| Ảnh được AI check | **Chỉ ảnh đầu tiên** (`images[0]`) | Cần cân nhắc mở rộng sang tất cả ảnh |
...
- **Chỉ ảnh đầu tiên** (`images[0]`) được gửi qua AI để xác thực (xem mục 10 – Điểm cần cải thiện)
...
| **Số ảnh được AI check** | Chỉ `images[0]` | Nên kiểm tra **tất cả ảnh**, từ chối nếu có bất kỳ ảnh nào AI không nhận diện được |
